...
def create_reference_list(
    ...
    if response.status_code != 200:
        raise APIError(f"Failed to create reference list '{name}': {response.status_code} {response.text}")
...
def get_reference_list(
    ...
    if response.status_code != 200:
        raise APIError(f"Failed to get reference list '{name}': {response.status_code} {response.text}")
...
def list_reference_lists(
    ...
    while True:
        ...
        if response.status_code != 200:
            raise APIError(f"Failed to list reference lists: {response.status_code} {response.text}")
...
def update_reference_list(
    ...
    if description is None and entries is None:
        raise SecOpsError("Either description or entries (or both) must be provided for update.")
    ...
    if response.status_code != 200:
        raise APIError(f"Failed to update reference list '{name}': {response.status_code} {response.text}")
