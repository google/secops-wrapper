...
def test_curated_rule_set_categories(chronicle):
    ...
    # Test basic listing
    categories = chronicle.list_curated_rule_set_categories(as_list=True)
...
def test_curated_rule_set_deployments(chronicle):
    ...
    assert deployments.get("curatedRuleSetDeployments")
    assert isinstance(deployments.get("curatedRuleSetDeployments"), list)

    if not deployments:
        pytest.skip("No rule set deployments found to test with")

    # Test with filters
    list_enabled_rules_result = chronicle.list_curated_rule_set_deployments(
        only_enabled=True
    )
    enabled_deployments = list_enabled_rules_result.get(
    ...
    list_alert_rules_result = chronicle.list_curated_rule_set_deployments(
        only_alerting=True
    )
    alerting_deployments = list_alert_rules_result.get(
        "curatedRuleSetDeployments"
    )
    ...
    first_deployment = deployments.get("curatedRuleSetDeployments")[0]
    ...
    rule_sets_result = chronicle.list_curated_rule_sets()
    rule_sets = rule_sets_result.get("curatedRuleSets")
...
def test_update_curated_rule_set_deployment(chronicle):
    ...
    assert rule_sets.get("curatedRuleSets"), "No rule sets found to test with"

    # Get a rule set ID
    first_rule_set = rule_sets["curatedRuleSets"][0]
...
def test_search_curated_detections(chronicle):
    ...
    assert rules.get("curatedRules"), "No curated rules found to test with"
    first_rule = rules["curatedRules"][0]
    rule_id = first_rule["name"].split("/")[-1]
