...
class ChronicleClient:
    ...
    def get_stats(
        ...
        payload = {
            ...
            "fieldAggregations": {"maxValuesPerField": max_values},
        ...
        while attempt < max_attempts:
            ...
            # Check both possible paths for completion status
                ...
                results.get("done")  # Check top level
                # Check under operation
                or results.get("operation", {}).get("done")
                # Check under response
                or results.get("response", {}).get("complete")
            )

            if done:
                # Check both possible paths for stats
                stats = results.get("response", {}).get(
                    "stats"
                ) or results.get(  # Check under response
                    "operation", {}
                ).get(
                    "response", {}
                ).get(
                    "stats"
                )  # Check under operation.response
                if stats:
    ...
    def search_udm(
        ...
        while attempt < max_attempts:
            ...
            if done:
                events = results.get("response", {}).get("events", {}).get(
                    "events", []
                ) or results.get("operation", {}).get("response", {}).get(
                    "events", {}
                ).get(
                    "events", []
                )
            ...
            timeline = (
                Timeline(
                    buckets=[
                        ...
                        for bucket in timeline_data.get("buckets", [])
                    ],
    ...
    def list_iocs(
        ...
        url = (
            f"{self.base_url}/{self.instance_id}/legacy:legacySearchEnterpriseWideIoCs"
        )
    ...
    def _process_alerts_response(self, response) -> list:
        ...
        if line_buffer:
            try:
                ...
                if not updates:
                    raise APIError(
                        f"Failed to parse alerts response: {str(e)} - Data: {current_buffer}"
                    )
    ...
    def get_detections(
        ...
        try:
            ...
            return LegacySearchDetectionsResponse.from_dict(response_content)
    ...
    def list_rules(
        ...
        try:
            # Parse the array of detections
            response_content = json.loads(response.content)
            return ListRulesResponse.from_dict(response_content)
    ...
    def get_rule(self, name: str, view: RuleView = RuleView.BASIC) -> Rule:
        ...
        try:
            ...
            response_content = json.loads(response.content)
            return Rule.from_dict(response_content)
    ...
    def verify_rule(self, rule_text: str) -> CompilationResult:
        """Verifies the given rule text as UTF8 string."""
        url = f"{self.base_url}/{self.instance_id}:verifyRuleText"
        ...
        if response.status_code != 200:
            ...
            raise Exception(
                f"Failed to verify rule: {response.text}"
            )  # Use a more specific exception if needed

        try:
            # Parse the JSON response and create a CompilationResult object
            response_content = json.loads(response.content)
            return CompilationResult.from_dict(response_content)
